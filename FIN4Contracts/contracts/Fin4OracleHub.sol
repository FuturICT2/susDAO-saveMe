...
contract Fin4OracleHub {
    ...
    mapping (string => address[]) public sensorIDtoSubscribers;  //receives a String and maps to an addres. The ID string mapped to the Metamask address (value-key) pair

    // msg.sender is the oracle: our Fin4OracleEngine server or later sensors that talk directly to smart contracts
    function receiveSensorSignal(string memory sensorID, uint timestamp, string memory data) public { //parse all the received data from the sensors in the function
        // require(registeredOracles(msg.sender) ... #ConceptualDecision
        for (uint i = 0; i < sensorIDtoSubscribers[sensorID].length; i++) {  //we go through the whole list of subscribed senders  
